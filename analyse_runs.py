...
def plot_metric(agg_df, metric, ylabel, suffix, project):
    ...
    plt.xlabel("Training samples", fontsize=28)
    plt.ylabel(ylabel, fontsize=28)
    #plt.title(f"{ylabel} in Abhängigkeit der Trainingssamples", fontsize=28)
    plt.xticks(grouped["train_samples"], rotation=-45)
    plt.grid(True)
    plt.minorticks_on()
...
# Für jedes Projekt ausführen
api = wandb.Api()

for project in projects:
    print(f"\n🔄 Verarbeite Projekt: {project}")
    ...
    # Metriken in Prozent umrechnen (außer test_accuracy – die ist schon in %)
    ...
    # Plots erstellen
    plot_metric(agg_df, "test_accuracy", "Test Accuracy / %", "Accuracy_mean", project)
    plot_metric(agg_df, "test_sensitivity", "Test Sensitivity / %", "Sensitivity_mean", project)
    plot_metric(agg_df, "test_specificity", "Test Specificity / %", "Specificity_mean", project)
