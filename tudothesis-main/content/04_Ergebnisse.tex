...
Das Netzwerk wurde mit unterschiedlichen Datensatzgrößen trainiert die Accuracy, Sensitivity und Specificity des Testdatensatzes berechnet.
Da beobachtet wurde, dass die Metrikwerte bei 2723 gesunken sind, wurde zusätzlich das Netzwerk mit 2553, 2893 und 3234 
Trainingssamples trainiert und das Netzwerk an schließend auf den Testdatensatz angewendet.
...
Der Anstieg verläuft bis etwa 1362 samples relativ konstant. Bei 1702 Training samples singt die Accuracy auf $\qty{93.1157}{\%}$ und steigt darauf hin wieder an und geht in Plateau über.
Die Standardabweichungen der Accuracy liegen im allgemeinem im $\qty{1}{\%}$ Bereich.
Ausgenommen sind dabei die Wert bei einer Verwendeten Sample Anzahl von 340, 1021 und 2383.
Die Sensitivity zeigt wie die Accuracy einen kontinuierlichen anstieg von 340 bis 1362. 
Darauf hin sinkt sie auf 0.9147 ab, bei 1702 Training samples. Dabei wurde auch die geringste Standardabweichung von 0.0078 berechnet.
steigt jedoch wieder auf 0.9314 bei 2723 Training samples.
Die geringste Sensitivity liegt bei 0.8606, welche bei 340 Bildern auftritt. Bei der Verwendung aller Training samples wird der höchste Wert mit 0.9462 aufgenommen.
Der Unterschied zwischen dem Minimalwert und Maximalwert der Sensitivity 0.0856.
Die Standardabweichung im Plateau liegen zwischen 0.0134 und 0.0230.
Die Specificity steigt zwischen 340 und 2042 kontinuierlich von 0.8644 auf 0.9741 an und geht anschließend ins Plateau.
...
Zwischen dem maximalen Specificity und der geringsten liegt eine Differenz von 0.1228.
Die größte Standardabweichung von 0.0329 wurde bei der Verwendung von 681 Training samples detektiert.
Im Plateau schwangt die Standardabweichung zwischen 0.0080 und 0.0237 
\subsection{Augmentation}

Die Abbildung \ref{fig:augmentation_tu} zeigt den Verlauf der Metriken in Abhängigkeit der verwendeten Training samples.
\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
...
\end{figure}
Die jeweiligen Mittelwerte und Standardabweichungen sind in der Tabelle \ref{tab:augm-tunotu} dargestellt.
\begin{table}[H]
    \centering
        \begin{tabular}{cccc}
            \toprule
...
Die Accuracy steigt mit der Anzahl der Samples dabei konstant an. 
Die Sensitivity steigt bis 1021 samples an und bleibt anschließend relativ konstant und erreicht anschließend das Plateau bei ungefähr 0.94.
Die Sättigung der Accuracy liegt bei $\qty{95}{\%}$ und die der Specificity bei 0.97.
Alle drei Metriken erreichen ihren niedrigsten Wert bei der Verwendung von 340 samples.

\subsection{Reduzierung der Tumor samples}

In der Abbildung \ref{fig:reduzierung_tu} wird die Accuracy, Sensitivity und Specificity in Abhängigkeit der Tumor samples abgebildet.
\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
