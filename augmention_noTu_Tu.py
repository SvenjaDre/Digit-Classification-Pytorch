...
# Dataset: no_tumor vs tumor (mit optionaler Flip-Augmentierung)
class CustomImageDataset(Dataset):
    def __init__(self, root_dir, apply_random_flip=False):
        self.image_paths = []
        self.labels = []
    ...
    def __getitem__(self, idx):
        ...
        # Zufälliges Flippen (nur beim Training)
...
def train():
    wandb.init(project="Augm-Messungen-noTu-Tu")
    config = wandb.config
    ...
    full_dataset = CustomImageDataset(root_dir=TRAIN_DIR, apply_random_flip=False)
    ...
    # Flip nur im Training aktivieren
    base_train_dataset.dataset.apply_random_flip = True
    val_dataset.dataset.apply_random_flip = False
...
def evaluate_on_test_data(model_path):
    ...
    test_dataset = CustomImageDataset(root_dir=TEST_DIR, apply_random_flip=False)
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project="Augm-Messungen-noTu-Tu")
